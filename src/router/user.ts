...
import {socketRepository, userRepository} from "../utils/db"
    ...
    router.delete('/admin/users/:id', (req: Request, res: Response, next: NextFunction) => auth(req, res, next), async (req: Request, res: Response) => {
        try {
            if (req.body.user.role === 'admin') {
                ...
                const existUser = await userRepository.findOne({where: {id: parseInt(req.params.id)}})
                if (existUser !== null) {
    ...
    router.delete('/admin/room', (req: Request, res: Response, next: NextFunction) => auth(req, res, next), async (req: Request, res: Response) => {
        try {
            if (req.body.user.role === 'admin') {
                ...
                const existRoom = await socketRepository.createQueryBuilder('socket').select(['socket.roomName']).groupBy("socket.roomName").getMany()
                if (existRoom.length !== 0) {
